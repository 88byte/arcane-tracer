...
async function sendIngestionEmail(pullId, { source, pull_type, status, recordsFetched, recordsCreated, recordsUpdated, durationMs, errorMessage, apiCallsUsed, apiBudgetRemaining }) {
  try {
    ...
    // Optional API-budget surfacing: rendered only when the caller supplies
    // apiCallsUsed. Format: "X api_calls used, Y budget remaining for today".
    const budgetLine = (typeof apiCallsUsed === 'number')
      ? `API Budget: ${apiCallsUsed} api_calls used${typeof apiBudgetRemaining === 'number' ? `, ${apiBudgetRemaining} budget remaining for today` : ''}`
      : null;
    const body = [
      `Pull ID: ${pullId}`,
      `Source: ${source}`,
...
// plus OFAC SDN refresh and IRS 990 enrichment.
app.get('/api/cron/daily', requireCronSecret, async (req, res) => {
  try {
    ...
    // 4. IRS 990 enrichment (ProPublica Nonprofit Explorer)
...
app.get('/api/cron/weekly', requireCronSecret, async (req, res) => {
  try {
    ...
    // FAPIIS is no longer published as a standalone bulk file. The call
    // below runs a no-op stub that marks the pull completed with a skipped
    // flag so cron history stays consistent. See ingestFAPIIS for details.
...
// FAPIIS is not currently published as a standalone bulk file that we
// can ingest without an account-gated login. We keep this function and
// its endpoint as a thin stub so cron history, feature wiring, and
// downstream callers all continue to work without change. The stub
// marks the pull completed with a skipped flag.
async function ingestFAPIIS(pullId) {
  console.log('[FAPIIS] Standalone FAPIIS pull not currently wired; marking skipped.');
  await supabase.from('data_pulls').update({
    status: 'completed',
    completed_at: new Date().toISOString(),
    records_fetched: 0,
    ...
    metadata: {
      skipped: true,
      reason: 'fapiis_bulk_not_wired',
      note: 'FAPIIS bulk ingestion is not currently implemented.'
    }
  }).eq('id', pullId);
